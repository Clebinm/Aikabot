...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      if (isNaN(args[0])) return message.channel.send(":x:Id invalido deste usuário tente novamente, Acho que desta vez vai funcionar!.")


      user = message.mentions.members.first() || await message.guild.members.fetch(args[0]).catch(err => { return message.channel.send(":x: Não consegui achar este usuário!") })
    }

    if (!user) {
      return message.channel.send(":x: Novamente não consegui acha-lo corrija o codigo é tente novamente!")
    }
    ...
    if (user.user.presence.activities.length) {
      ...
      embed.addField("Joined At", moment(user.user.joinedAt).format("LLLL"))
        .addField("Conta criada em", moment(user.user.createdAt).format("LLLL"))
